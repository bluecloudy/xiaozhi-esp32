...
private:
        ...
        for (size_t i = 0; i < sizeof(camera_addresses); i++) {
            ...
            if (ret == ESP_OK) {
                ...
                if (ret == ESP_OK && (data[0] != 0 || data[1] != 0)) {
                    ...
                    ESP_LOGI(TAG, "Camera detected (OV2640 mode) PID=0x%04X (addr=0x%02X)",
                             detected_pid, addr);
                    camera_found = true;
                ...
                if (ret == ESP_OK) {
                    ...
                    if (ret == ESP_OK) {
                        ...
                        if (detected_pid != 0) {
                            ESP_LOGI(TAG, "Camera detected (OV3660 mode) PID=0x%04X (addr=0x%02X)",
                                     detected_pid, addr);
                            camera_found = true;
                            i2c_master_bus_rm_device(dev_handle);
                            break;
    ...
    void InitializeLcdDisplay() {
        ...
        display_ = new MimiEmojiDisplay(panel_io, panel, DISPLAY_WIDTH, DISPLAY_HEIGHT,
    ...
    void InitializeMimiController() { ::InitializeMimiController(hw_config_); }
...
public:
    MimiRobot()
        ...
        if (!has_camera_) {
            has_camera_ = true;
            camera_type_ = MIMI_CAMERA_UNKNOWN;
            ESP_LOGW(TAG, "Camera configuration forced, but camera type could not be detected");
            i2c_master_bus_config_t i2c_bus_cfg = {
                .i2c_port = I2C_NUM_0,
                .sda_io_num = CAMERA_VERSION_CONFIG.i2c_sda_pin,
                .scl_io_num = CAMERA_VERSION_CONFIG.i2c_scl_pin,
            ...
            ESP_LOGI(TAG, "Camera configuration forced");
        }
#elif MIMI_HARDWARE_VERSION == MIMI_VERSION_NO_CAMERA
        has_camera_ = false;
        camera_type_ = MIMI_CAMERA_NONE;
        ESP_LOGI(TAG, "No-camera configuration forced");
#else
#error "MIMI_HARDWARE_VERSION is invalid"
#endif
