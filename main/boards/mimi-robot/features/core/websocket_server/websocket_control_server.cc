...
static const char* kWsTag = "WSControl";
...
esp_err_t WebSocketControlServer::ws_handler(httpd_req_t *req) {
    ...
    if (req->method == HTTP_GET) {
        ESP_LOGI(kWsTag, "Handshake done, the new connection was opened");
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(kWsTag, "httpd_ws_recv_frame failed to get frame len with %d", ret);
        return ret;
    }
    ESP_LOGI(kWsTag, "frame len is %d", ws_pkt.len);
    
    if (ws_pkt.len) {
        buf = (uint8_t*)calloc(1, ws_pkt.len + 1);
        if (buf == NULL) {
            ESP_LOGE(kWsTag, "Failed to calloc memory for buf");
            return ESP_ERR_NO_MEM;
        }
        ws_pkt.payload = buf;
        ret = httpd_ws_recv_frame(req, &ws_pkt, ws_pkt.len);
        if (ret != ESP_OK) {
            ESP_LOGE(kWsTag, "httpd_ws_recv_frame failed with %d", ret);
            free(buf);
            return ret;
        }
        ESP_LOGI(kWsTag, "Got packet with message: %s", ws_pkt.payload);
    }
    
    ESP_LOGI(kWsTag, "Packet type: %d", ws_pkt.type);
    
    if (ws_pkt.type == HTTPD_WS_TYPE_CLOSE) {
        ESP_LOGI(kWsTag, "WebSocket close frame received");
    ...
    if (ws_pkt.type == HTTPD_WS_TYPE_TEXT) {
        ...
        ESP_LOGW(kWsTag, "Unsupported frame type: %d", ws_pkt.type);
    }
...
bool WebSocketControlServer::Start(int port) {
    ...
    if (httpd_start(&server_handle_, &config) == ESP_OK) {
        ...
        ESP_LOGI(kWsTag, "WebSocket server started on port %d", port);
        return true;
    }

    ESP_LOGE(kWsTag, "Failed to start WebSocket server");
    return false;
}
...
void WebSocketControlServer::Stop() {
    if (server_handle_) {
        ...
        clients_.clear();
        ESP_LOGI(kWsTag, "WebSocket server stopped");
    }
}

void WebSocketControlServer::HandleMessage(httpd_req_t *req, const char* data, size_t len) {
    if (data == nullptr || len == 0) {
        ESP_LOGE(kWsTag, "Invalid message: data is null or len is 0");
        return;
    }
    
    if (len > 4096) {
        ESP_LOGE(kWsTag, "Message too long: %zu bytes", len);
        return;
    }
    
    char* temp_buf = (char*)malloc(len + 1);
    if (temp_buf == nullptr) {
        ESP_LOGE(kWsTag, "Failed to allocate memory");
        return;
    }
    ...
    if (root == nullptr) {
        ESP_LOGE(kWsTag, "Failed to parse JSON");
        return;
    }
    ...
    if (payload == nullptr) {
        ESP_LOGE(kWsTag, "Invalid message format or failed to parse");
    }
...
void WebSocketControlServer::AddClient(httpd_req_t *req) {
    ...
    if (clients_.find(sock_fd) == clients_.end()) {
        ...
        ESP_LOGI(kWsTag, "Client connected: %d (total: %zu)", sock_fd, clients_.size());
    }
}
...
void WebSocketControlServer::RemoveClient(httpd_req_t *req) {
    ...
    ESP_LOGI(kWsTag, "Client disconnected: %d (total: %zu)", sock_fd, clients_.size());
}
