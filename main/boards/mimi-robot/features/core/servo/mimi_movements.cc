...
void Mimi::Init(int left_leg, int right_leg, int left_foot, int right_foot, int left_hand,
                int right_hand) {
...
void Mimi::SetTrims(int left_leg, int right_leg, int left_foot, int right_foot, int left_hand,
                    int right_hand) {
...
void Mimi::MoveServos(int time, int servo_target[]) {
...
void Mimi::MoveSingle(int position, int servo_number) {
    if (position > 180)
        position = 90;
    if (position < 0)
        position = 90;
...
void Mimi::OscillateServos(int amplitude[SERVO_COUNT], int offset[SERVO_COUNT], int period,
...
void Mimi::Execute(int amplitude[SERVO_COUNT], int offset[SERVO_COUNT], int period,
...
void Mimi::Execute2(int amplitude[SERVO_COUNT], int center_angle[SERVO_COUNT], int period,
                    double phase_diff[SERVO_COUNT], float steps) {
    if (GetRestState() == true) {
        SetRestState(false);
    }
...
void Mimi::Home(bool hands_down) {
    if (is_mimi_resting_ == false) {
        ...
        is_mimi_resting_ = true;
    }

    vTaskDelay(pdMS_TO_TICKS(200));
}

bool Mimi::GetRestState() {
    return is_mimi_resting_;
}

void Mimi::SetRestState(bool state) {
    is_mimi_resting_ = state;
}
...
void Mimi::AscendingTurn(float steps, int period, int height) {
...
void Mimi::HandsUp(int period, int dir) {
    if (!has_hands_) {
        return;
    }
...
void Mimi::HandsDown(int period, int dir) {
    if (!has_hands_) {
        return;
    }
...
void Mimi::Windmill(float steps, int period, int amplitude) {
    if (!has_hands_) {
        return;
    }
...
void Mimi::Takeoff(float steps, int period, int amplitude) {
    if (!has_hands_) {
        return;
    }
...
void Mimi::Fitness(float steps, int period, int amplitude) {
    if (!has_hands_) {
        return;
    }
...
void Mimi::Greeting(int dir, float steps) {
    if (!has_hands_) {
        return;
    }
    if (dir == LEFT) {
...
void Mimi::Shy(int dir, float steps) {
    if (!has_hands_) {
        return;
    }
...
void Mimi::EnableServoLimit(int diff_limit) {
    for (int i = 0; i < SERVO_COUNT; i++) {
        if (servo_pins_[i] != -1) {
