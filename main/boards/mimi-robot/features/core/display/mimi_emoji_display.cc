...
#define TAG "MimiEmojiDisplay"
MimiEmojiDisplay::MimiEmojiDisplay(esp_lcd_panel_io_handle_t panel_io, esp_lcd_panel_handle_t panel, int width, int height, int offset_x, int offset_y, bool mirror_x, bool mirror_y, bool swap_xy)
    : SpiLcdDisplay(panel_io, panel, width, height, offset_x, offset_y, mirror_x, mirror_y, swap_xy) {
}

void MimiEmojiDisplay::SetupUI() {
    if (setup_ui_called_) {
...
void MimiEmojiDisplay::SetupPreviewImage() {
    DisplayLockGuard lock(this);
...
void MimiEmojiDisplay::InitializeMimiEmojis() {
    ESP_LOGI(TAG, "Mimi emoji initialization is handled by the Assets system");
}

LV_FONT_DECLARE(MIMI_ICON_FONT);
void MimiEmojiDisplay::SetStatus(const char* status) {
    ...
    if (strcmp(status, Lang::Strings::LISTENING) == 0) {
        lv_obj_set_style_text_font(status_label_, &MIMI_ICON_FONT, 0);
...
void MimiEmojiDisplay::SetPreviewImage(std::unique_ptr<LvglImage> image) {
    DisplayLockGuard lock(this);
    if (preview_image_ == nullptr) {
